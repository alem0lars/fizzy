...
describe Fizzy::Sync::Git do
  ...
  just_in_docker do
    ...
    let(:git) { described_class.new(local_dir_path, repo_url) }
    ...
    describe "#local_changed?" do
      ...
      context "when there are local modifications" do
        before { local_dir_path.join("qwerty").write("lol") }
    ...
    describe "#working_tree_changes?" do
      ...
      context "when there are local modifications" do
        before { local_dir_path.join("foo").write("foo") }
        it { is_expected.to eq(true) }

        context "and commit is performed" do
          before { git.send :perform_commit, message: "xD" }
          it { is_expected.to eq(false) }

          context "and then another file is added" do
            before { local_dir_path.join("bar").write("bar") }
            it { is_expected.to eq(true) }
          end
        end
      end
